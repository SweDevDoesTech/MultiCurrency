...
public class Inventory_CreateCurrency {
    ...
    public Inventory_CreateCurrency(Player player) {
        ...
        gui.setCloseGuiAction(event -> SchedulerUtils.runLater(1L, () -> MultiCurrency.getInventoryManager().getMainInventory().openInventory(player)));

        // Create
        String createLoreSingular = MultiCurrency.getInventoryCache().getCurrencySingular().get(player) == null
                ? "&7Singular: UNDEFINED"
                : "&7Singular: " + MultiCurrency.getInventoryCache().getCurrencySingular().get(player);

        String createLorePlural = MultiCurrency.getInventoryCache().getCurrencyPlural().get(player) == null
                ? "&7Plural: UNDEFINED"
                : "&7Plural: " + MultiCurrency.getInventoryCache().getCurrencyPlural().get(player);
        ...
        String createLorePayable = "&7Payable: &cFALSE";
        ...
        String createLoreDefault = "&7Default: &cFALSE";
        ...
        createItemLore.add(Utils.colorize(createLoreSingular));
        createItemLore.add(Utils.colorize(createLorePlural));
    ...
    public void updateCreationLore() {
        if (MultiCurrency.getInventoryCache().getCurrencySingular().get(player) != null)
            createItemLore.set(1, Utils.colorize("&7Singular: " + MultiCurrency.getInventoryCache().getCurrencySingular().get(player)));

        if (MultiCurrency.getInventoryCache().getCurrencyPlural().get(player) != null)
            createItemLore.set(2, Utils.colorize("&7Plural: " + MultiCurrency.getInventoryCache().getCurrencyPlural().get(player)));

        if (MultiCurrency.getInventoryCache().getSymbol().get(player) != null)
            createItemLore.set(3, Utils.colorize("&7Symbol: " + MultiCurrency.getInventoryCache().getSymbol().get(player)));

        if (MultiCurrency.getInventoryCache().getDefaultBalance().get(player) != null)
            createItemLore.set(4, Utils.colorize("&7Default Balance: " + MultiCurrency.getInventoryCache().getDefaultBalance().get(player)));

        if (MultiCurrency.getInventoryCache().getPayable().get(player) != null)
            if (!MultiCurrency.getInventoryCache().getPayable().get(player))
                createItemLore.set(5, Utils.colorize("&7Payable: &c" + String.valueOf(MultiCurrency.getInventoryCache().getPayable().get(player)).toUpperCase()));
            else
                createItemLore.set(5, Utils.colorize("&7Payable: &a" + String.valueOf(MultiCurrency.getInventoryCache().getPayable().get(player)).toUpperCase()));

        if (MultiCurrency.getInventoryCache().getIsDefault().get(player) != null)
            if (!MultiCurrency.getInventoryCache().getIsDefault().get(player))
                createItemLore.set(6, Utils.colorize("&7Default: &c" + String.valueOf(MultiCurrency.getInventoryCache().getIsDefault().get(player)).toUpperCase()));
            else
                createItemLore.set(6, Utils.colorize("&7Default: &a" + String.valueOf(MultiCurrency.getInventoryCache().getIsDefault().get(player)).toUpperCase()));

        if (MultiCurrency.getInventoryCache().getMaterial().get(player) != null)
            createItemLore.set(7, Utils.colorize("&7Material: " + String.valueOf(MultiCurrency.getInventoryCache().getMaterial().get(player)).toUpperCase()));
